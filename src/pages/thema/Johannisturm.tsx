...
/** Was die Zeichnung zu einem Eintrag zeigt. */
function zustand(e: Eintrag) {
  const stand = e.stand ?? VOLLENDET;
  return {
    ...stand,
    hoehe: e.ort ? orte[e.ort].hoehe : stand.turm * TURM_HOEHE,
...
export function Johannisturm() {
  ...
  const a = zustand(chronik[aktiv]);
  const b = zustand(chronik[Math.min(aktiv + 1, chronik.length - 1)]);
  ...
  const bild = {
    schiff: lerp(a.schiff, b.schiff, t),
    turm: lerp(a.turm, b.turm, t),
    geruest: lerp(a.geruest, b.geruest, t),
    hoehe: lerp(a.hoehe, b.hoehe, t),
    markeX: lerp(a.markeX, b.markeX, t),
    marke: lerp(a.marke, b.marke, t),
  };
  ...
  return (
    <PageLayout>
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 lg:grid-cols-2 lg:gap-16 lg:px-8">
          ...
          <div>
            ...
            <ul className="divide-y divide-ink-line/60 border-y border-ink-line/60">
              {glocken.map((g) => (
                <li key={g.name} className="grid grid-cols-[3.5rem_minmax(0,1fr)_auto] items-baseline gap-4 py-3">
                  <span className="font-display text-lg tabular-nums text-ink">{g.jahr}</span>
                  <span>
                    <span className="font-medium text-ink">{g.name}</span>
                    {g.hinweis && <span className="mt-0.5 block text-sm text-ink-soft">{g.hinweis}</span>}
                  </span>
                  <span className="text-sm tabular-nums text-ink-soft">{g.kg} kg</span>
                </li>
              ))}
            </ul>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 lg:px-8">
        <div className="grid grid-cols-[4.5rem_minmax(0,1fr)] gap-5 sm:grid-cols-[13rem_minmax(0,1fr)] sm:gap-10 lg:grid-cols-[22rem_minmax(0,1fr)] lg:gap-16">
          <div>
            <div className="sticky top-[calc(85px+1rem)] flex flex-col sm:h-[calc(100svh-85px-2.5rem)] sm:py-6">
              <div className="mb-3 sm:mb-5 sm:pl-24 lg:pl-32" aria-hidden="true">
                <div className="font-display text-xl tabular-nums leading-none text-ink sm:text-4xl">
                  {bild.hoehe > TURM_HOEHE - 0.05
                    ? TURM_HOEHE.toLocaleString("de-DE")
                    : Math.round(bild.hoehe)}
                  <span className="ml-1 text-sm sm:text-xl">m</span>
                </div>
                <div className="mt-1 text-[11px] leading-tight text-ink-soft sm:text-sm">
                  {imBau ? "Bauhöhe" : ort ? orte[ort].label : "Turmspitze"}
                </div>
              </div>
              <div className="min-h-0 sm:flex-1 sm:pl-24 lg:pl-32">
                <JohannisturmZeichnung
                  stand={bild}
                  hoehe={bild.hoehe}
                  markeX={bild.markeX}
                  marke={bild.marke}
              ...
              <p className="mt-3 hidden text-xs leading-snug text-ink-soft sm:block sm:pl-24 lg:pl-32">
                Platzhalter-Zeichnung. Höhen im Turm geschätzt.
              </p>
            </div>
          </div>

          <div className="max-w-2xl pb-16 pt-12 lg:pb-24 lg:pt-16">
