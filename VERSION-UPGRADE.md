...
| `@types/node` | ^25.6.0 | ^26.x | Welle 1 — nur Typen, kein Laufzeitverhalten |
| `vite` | ^6.0.0 | ^8.x | Welle 2 — Build-Tool-Major |

## Besonderheit

**Achtung `react-map-gl`.** Steht auf `^8.1.1`, das Lockfile haelt 8.1.1. Version 8.1.2 aendert den Typ von `maxBounds` und bricht `StadtKarte.tsx`, `MoosburgMap.tsx` und `StrassenKarte.tsx`. Beim Update entweder auf 8.1.1 halten oder die drei Aufrufstellen auf den neuen Typ anpassen — aber bewusst, nicht nebenbei.
